...
export const getPublicHostel = cache(async function getPublicHostel(slug: string): Promise<{ hostel?: PublicHostelDetail; error?: string }> {
  try {
    ...
    const [{ data: profile }, { data: rooms }, { data: foodItems }, { data: pkgConfig }] = await Promise.all([
      ...
      admin
        .from("hms_package_configs")
        .select("id,hostel_id,food_monthly_rate,food_bd_rate,food_3meals_rate,food_breakfast_rate,food_lunch_rate,food_dinner_rate,food_all_meals_rate,ac_per_unit_rate,ac_charge_label,security_deposit,registration_fee,ac_maintenance_rate,notice_period_days,package_prices,seater_prices,washroom_premium,created_at,updated_at")
        .eq("hostel_id", hostelId)
        .maybeSingle(),
    ]);
