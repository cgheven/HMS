-- Manual discounts, percentage of RENT only.
--
-- Two sources, one stored number:
--   hms_tenants.discount_percent   — the standing concession agreed at admission.
--     22,000 rent at 10% bills 19,800 every month until it is changed. The rent
--     and the percentage are stored SEPARATELY rather than baking 19,800 in, so
--     the reduction stays visible on the receipt and countable in reports.
--   hms_payments.manual_discount_percent — a one-off on a single bill, typed in
--     the Pay dialog when a tenant was away most of the month.
--
-- They stack, and the sum is clamped to the rent the referral discount has not
-- already taken. A discount NEVER touches food, metered AC, the deposit, the
-- registration fee or AC maintenance — the same clamp the referral discount has
-- had since migration 186, and what makes a negative bill impossible by
-- construction rather than by a check.
--
-- The function below is the LIVE production definition (pg_get_functiondef,
-- 2026-08-31) with only the marked additions. Nothing else is touched.
--
-- STAGE ONLY for now. Not applied to production, deliberately.
--
-- DEPLOY ORDER: the columns first, then this function, then the code — the
-- function references hms_tenants.discount_percent and assigns
-- new.discount_percent, so it cannot be installed before they exist.

alter table public.hms_tenants
  add column if not exists discount_percent numeric(5,2)
  constraint hms_tenants_discount_percent_range check (discount_percent is null or (discount_percent >= 0 and discount_percent <= 100));

comment on column public.hms_tenants.discount_percent is
  'Standing discount agreed at admission, as a percentage of monthly rent. NULL = none. Applied to every unpaid bill; collected bills keep the percentage they were collected with.';

alter table public.hms_payments
  add column if not exists manual_discount_percent numeric(5,2)
  constraint hms_payments_manual_discount_percent_range check (manual_discount_percent is null or (manual_discount_percent >= 0 and manual_discount_percent <= 100)),
  add column if not exists discount_percent numeric(5,2) not null default 0,
  add column if not exists discount_amount numeric(10,2) not null default 0;

comment on column public.hms_payments.manual_discount_percent is
  'One-off discount typed in the Pay dialog for THIS bill only. NULL = none.';
comment on column public.hms_payments.discount_percent is
  'Total percentage applied to this bill: the tenant standing discount plus manual_discount_percent. Derived by hms_recalculate_payment_amount; pinned once the bill is collected.';
comment on column public.hms_payments.discount_amount is
  'Rupee value of discount_percent against this bill''s rent. amount is stored NET of it.';

-- Partial index: the reports "who is on a discount" query, and small on every
-- branch that has never given one.
create index if not exists hms_tenants_discount_percent_idx
  on public.hms_tenants (hostel_id) where discount_percent is not null;
create index if not exists hms_payments_discount_idx
  on public.hms_payments (hostel_id, for_month) where discount_amount > 0;
