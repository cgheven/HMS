-- Stop already-collected bills from being silently re-priced.
--
-- hms_recalculate_payment_amount() fires BEFORE INSERT OR UPDATE on every
-- hms_payments row and re-derives amount, food_charge and ac_maintenance_charge
-- from CURRENT tenant/package/room state, with no regard for whether the money
-- was already taken. So raising a tenant's rent and then touching any of their
-- old paid bills — marking, editing, applying AC units — rewrote that bill at
-- today's price. 16 collected rows across 6 hostels were sitting exposed,
-- worst cases a paid Rs 24,000 bill that would rewrite to Rs 0 and a pro-rated
-- Rs 2,933 that would rewrite to Rs 22,000.
--
-- Only the monthly branch changes. Inserts, pending bills, daily tenants,
-- reservations and the pending -> paid collection path are all untouched.
