-- Migration 256: UNDO reverses the one-off discount.
--
-- Before: undoing a discounted payment left the bill "partially_paid" at zero
-- collected but STILL discounted (the price-freeze also froze the one-off), and
-- the discount could not be removed. Now a full undo clears the one-off, so the
-- bill reopens at its full price and can be re-collected (with a fresh discount).
--
-- The one-off is frozen ONLY while money is genuinely held. The base PRICE, the
-- standing discount and the referral discount stay frozen exactly as before.
-- BYTE-IDENTICAL: every existing bill recomputes to the same amount (verified by
-- a rolled-back recompute-diff) — money is only released when an undo explicitly
-- reverses it.
...
AS $function$
 DECLARE
   ...
   -- Migration 256: TRUE only when money is genuinely held both before and after
   -- this write. Freezes the one-off discount for a live collected bill, while an
   -- UNDO to zero collected (and a re-collection of a reopened bill) does NOT
   -- freeze it — so undo reverses the one-off and a reopened bill takes a fresh one.
   v_pin_oneoff         boolean := false;
 BEGIN
   ...
   IF TG_OP = 'UPDATE'
     ...
     -- Migration 256: freeze the one-off discount (rupees + percent) ONLY when
     -- money is genuinely held both before AND after this write. An UNDO to zero
     -- collected leaves the column as the undo set it (NULL → the one-off is
     -- reversed), and a re-collection of a reopened bill (old.amount_paid = 0)
     -- takes the operator's freshly-entered one-off — while a normal collected
     -- bill (and a bare re-write of one) keeps exactly what it was collected with.
     -- The standing discount, referral and the base PRICE stay frozen regardless.
     IF coalesce(old.amount_paid, 0) > 0 AND coalesce(new.amount_paid, 0) > 0 THEN
       v_pin_oneoff := true;
   ...
   ELSE
     ...
     -- When the one-off is FROZEN (money held) keep the percent it was collected
     -- with (pinned above), so a re-printed historical receipt is unchanged.
     -- Otherwise — a fresh bill, or a reopened/re-collected one — derive the
     -- display % from the discountable subtotal. Migration 256.
     IF NOT v_pin_oneoff THEN
