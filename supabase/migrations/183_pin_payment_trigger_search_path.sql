-- 183: pin search_path on hms_recalculate_payment_amount().
--
-- The function is SECURITY DEFINER and had no pinned search_path (pg_proc.proconfig
-- was null), so it resolved hms_tenants / hms_package_configs / hms_rooms through
-- whatever search_path the calling session happened to carry. Every write to
-- hms_payments runs this body as the definer.
--
-- The body below is pg_get_functiondef() output captured from production verbatim.
-- Exactly one line is added: SET search_path. No other character differs — the point
-- of this migration shipping alone is that a checksum mismatch afterwards has one
-- possible cause, and it is not arithmetic.
--
-- Body references only three tables, all in public, and no extension functions, so
-- pinning to public alone resolves everything it needs.
