-- Migration 063: Fix bundle-only food add-on pricing in the payment trigger.
--
-- Bug (caught in a second review pass): the migration 062 fix only applied
-- the all-3-meals bundle rate when it was cheaper than the sum of the three
-- individual meal rates (v_all_meals_rate < v_addon_sum). For a hostel that
-- configures ONLY food_all_meals_rate (no individual breakfast/lunch/dinner
-- rates — a valid, intended configuration), v_addon_sum is always 0, so that
-- condition never holds and every tenant who ticks all 3 meals was charged
-- 0 instead of the bundle rate — free food, silently, on every sync.
--
-- Fix: apply the bundle rate whenever all three meals are ticked and a bundle
-- rate is configured, taking the cheaper of the two only when individual
-- rates actually add up to something (mirrors the corrected lib/food-addon.ts).
