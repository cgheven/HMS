-- 186: teach hms_recalculate_payment_amount() about the referral discount.
--
-- THE CONTRACT, IN ONE SENTENCE:
--   hms_payments.amount is stored NET of the referral discount, and the row's
--   GROSS is always (amount + referral_discount) — on every read, every write,
--   and every intermediate tuple the executor produces.
--
-- That identity is what makes the daily branch safe. The obvious alternative —
-- sniffing whether the caller "already baked in" a discount — is unreachable:
-- PostgreSQL specifies that the effects of per-row BEFORE INSERT triggers are
-- reflected in EXCLUDED, so on lib/monthly-payment-sync.ts's ON CONFLICT DO
-- UPDATE the second pass sees the FIRST pass's output, never the payload's
-- marker. The discount would compound silently on every monthly sync, and
-- hasStaleDailyRow compares only billed_days, so it would never self-heal.
--
-- The app-side rule that follows:
--   * a writer that computes amount from gross components sends
--     referral_discount: 0 alongside it ("nothing is baked in")
--   * a writer that derives amount from a value read out of the row sends no
--     referral_discount key at all (the stored one passes through)
-- Getting it wrong in the only possible direction INFLATES the bill — visible
-- and correctable, never a silent compounding under-bill.
--
-- Body is 183's, changed in seven places, all marked "referral phase 2" below.
-- For all 1,425 existing rows referral_discount and referral_percent are 0 and
-- hms_referral_rewards is empty, so every added expression collapses to the
-- arithmetic that is live today. Proven by differential checksum before ship.
